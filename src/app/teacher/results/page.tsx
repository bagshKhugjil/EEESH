...
export default function ResultsPage() {
  ...
  /** ---------- State ---------- */
  // Roster (анги, сурагчид) — meta/studentsList-ээс
  const [classes, setClasses] = useState<string[]>([]);
  const [students, setStudents] = useState<StudentLite[]>([]);
  const [rosterLoading, setRosterLoading] = useState(true);
  const [rosterErr, setRosterErr] = useState<string | null>(null);

  // Results (сонгосон шүүлтээр серверээс)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      if (!user) return;
      // Student таб: сурагч сонгогдоогүй бол дүн татахгүй
      ...
      try {
        ...
        const data = (await res.json()) as { subjects: string[]; results: ResultsByStudent; students?: StudentLite[]; classes?: string[] };
        if (!cancelled) {
  ...
  /** ---------- Fetch student profile when student changes (шинэ) ---------- */
  useEffect(() => {
    let cancelled = false;
    const run = async () => {
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <div className="header text-center pt-4">
        <div className="inline-flex gap-2 p-2 rounded-xl" style={{ background: "var(--card)", border: "1px solid var(--stroke)" }}>
          <Link href="/teacher" className="px-4 py-2 rounded-md font-bold" style={{ color: "var(--muted)" }}>Нүүр</Link>
          <Link href="/teacher/upload" className="px-4 py-2 rounded-md font-bold" style={{ color: "var(--muted)" }}>Дүн оруулах</Link>
          <Link href="/teacher/results" className="px-4 py-2 rounded-md font-bold" style={{ background: "var(--card2)", color: "var(--text)" }}>
            Дүн харах
          </Link>
          <Link href="/teacher/files" className="px-4 py-2 rounded-md font-bold" style={{ color: "var(--muted)" }}>Файл удирдах</Link>
        </div>
      </div>

      <div className="container max-w-[1200px] mx-auto px-6">
        <h1 className="text-center text-2xl font-extrabold mb-6">ЭЕШ-ийн Дүнгийн Нэгтгэл</h1>

        {/* Tabs */}
        <div className="tabs flex gap-2 border-b border-[var(--stroke)] mb-6 flex-wrap">
          <button
            className={`tab-link ${tab === "student" ? "active" : ""}`}
            style={{
              padding: "12px 20px",
              color: tab === "student" ? "var(--primary-bg)" : "var(--muted)",
              borderBottom: `3px solid ${tab === "student" ? "var(--primary-bg)" : "transparent"}`,
            }}
            onClick={() => setTab("student")}
          >Сурагчаар харах</button>
          <button
            className={`tab-link ${tab === "class" ? "active" : ""}`}
            style={{
              padding: "12px 20px",
              color: tab === "class" ? "var(--primary-bg)" : "var(--muted)",
              borderBottom: `3px solid ${tab === "class" ? "var(--primary-bg)" : "transparent"}`,
            }}
            onClick={() => setTab("class")}
          >Ангиар харах</button>
          <button
            className={`tab-link ${tab === "subject" ? "active" : ""}`}
            style={{
              padding: "12px 20px",
              color: tab === "subject" ? "var(--primary-bg)" : "var(--muted)",
              borderBottom: `3px solid ${tab === "subject" ? "var(--primary-bg)" : "transparent"}`,
            }}
            onClick={() => setTab("subject")}
          >Хичээлээр харах</button>
        </div>

        {/* Banners */}
        {rosterLoading && <div className="text-center text-[var(--muted)] py-4">Сурагчдын жагсаалт ачааллаж байна…</div>}
        {rosterErr && <div className="text-center text-red-400 py-4">{rosterErr}</div>}
        {resultsLoading && <div className="text-center text-[var(--muted)] py-2">Дүн шүүж байна…</div>}
        {resultsErr && <div className="text-center text-red-400 py-2">{resultsErr}</div>}
        ...
        {!rosterLoading && !rosterErr && (
          <>
            {/* Student View */}
            {tab === "student" && (
              <div className="card rounded-2xl p-6" style={{ background: "var(--card)", border: "1px solid var(--stroke)" }}>
                <h2 className="font-bold text-lg mb-4">Сурагчийн дүн хайх</h2>
                <div className="filters flex items-center gap-3 flex-wrap mb-4">
                  <label className="text-[var(--muted)] font-semibold">1. Анги:</label>
                  <Select
                    placeholder="Ангиа сонго"
                    value={svClass}
                    onChange={(v) => { setSvClass(v); setSvStudent(""); setSvSubject(""); }}
                    options={classes.map((c) => ({ value: c, label: c }))}
                  />
                  <label className="text-[var(--muted)] font-semibold">2. Сурагч:</label>
                  <Select
                    placeholder="Сурагчаа сонго"
                    value={svStudent}
                    onChange={(v) => { setSvStudent(v); setSvSubject(""); }}
                    options={students.filter((s) => (svClass ? s.class === svClass : true)).map((s) => ({ value: s.id, label: s.name }))}
                    disabled={!svClass}
                  />
                  <label className="text-[var(--muted)] font-semibold">3. Хичээл:</label>
                  <Select
                    placeholder="Хичээлээ сонго"
                    value={svSubject}
                    onChange={setSvSubject}
                  ...
                  <div id="student-overall-summary">
                    ...
                    <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))" }}>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Нийт дундаж оноо</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{studentOverall.average || "--"}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Нийт байр</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{studentOverall.rank}</div>
                      </div>
                    </div>
                  </div>
                )}
                ...
                {svStudent && svSubject && studentSubjectDetail && (
                  <>
                    <div className="mt-6">
                      ...
                      <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))" }}>
                        <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                          <div className="text-[var(--muted)] text-sm font-semibold mb-1">Миний оноо</div>
                          <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{studentSubjectDetail.myScore ?? "--"}</div>
                        </div>
                        <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                          <div className="text-[var(--muted)] text-sm font-semibold mb-1">Ангийн дундаж</div>
                          <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{studentSubjectDetail.classAvg ?? "--"}</div>
                        </div>
                        <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                          <div className="text-[var(--muted)] text-sm font-semibold mb-1">Ангид эзлэх байр</div>
                          <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{studentSubjectDetail.rank}</div>
                        </div>
                      </div>
                    </div>
                ...
                {!svStudent && <div className="text-center text-[var(--muted)] py-8">Дээрээс ангиа, дараа нь сурагчаа сонгоно уу.</div>}
              </div>
            )}
            ...
            {tab === "class" && (
              <div className="card rounded-2xl p-6" style={{ background: "var(--card)", border: "1px solid var(--stroke)" }}>
                <h2 className="font-bold text-lg mb-4">Ангийн дүнгийн нэгтгэл</h2>
                <div className="filters flex items-center gap-3 flex-wrap mb-4">
                  <label className="text-[var(--muted)] font-semibold">1. Анги:</label>
                  <Select
                    placeholder="Ангиа сонго"
                    ...
                    options={classes.map((c) => ({ value: c, label: c }))}
                  />
                  <label className="text-[var(--muted)] font-semibold">2. Хичээл:</label>
                  <Select
                    placeholder="Хичээлээ сонго"
                    value={cvSubject}
                    onChange={(v) => { setCvSubject(v); setCvDate(""); }}
                    options={cvSubjectOptions}
                    disabled={!cvClass}
                  />
                  <label className="text-[var(--muted)] font-semibold">3. Шалгалт (огноо):</label>
                  <Select
                    placeholder="Огноо сонго"
                    value={cvDate}
                    onChange={setCvDate}
                ...
                {cvSummary ? (
                  <>
                    ...
                    <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))" }}>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Ангийн дундаж</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{cvSummary.avg}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Хамгийн өндөр</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{cvSummary.top}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Хамгийн бага</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{cvSummary.low}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Хамрагдсан</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{cvSummary.count} сурагч</div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      ...
                      <ReactApexChart
                        ...
                        options={{
                          chart: { type: "bar", background: "transparent", height: 400, toolbar: { show: false } },
                          theme: { mode: themeMode },
                  ...
                  <div className="text-center text-[var(--muted)] py-8">Анги, хичээл, огноо сонгоно уу.</div>
                )}
              </div>
            )}
            ...
            {tab === "subject" && (
              <div className="card rounded-2xl p-6" style={{ background: "var(--card)", border: "1px solid var(--stroke)" }}>
                <h2 className="font-bold text-lg mb-4">Хичээлийн дүнгийн нэгтгэл</h2>
                <div className="filters flex items-center gap-3 flex-wrap mb-4">
                ...
                {subjectLeaderboard ? (
                  <>
                    ...
                    <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(200px,1fr))" }}>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Нийт дундаж</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{subjectLeaderboard.avg}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Хамгийн өндөр</div>
                        <div className="text-3xl font-extrabold text-[var(--primary-bg)]">{subjectLeaderboard.topScore}</div>
                      </div>
                      <div className="rounded-xl p-4 text-center" style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}>
                        <div className="text-[var(--muted)] text-sm font-semibold mb-1">Шилдэг сурагч</div>
                        <div className="text-2xl font-extrabold text-[var(--primary-bg)]">{subjectLeaderboard.topStudent}</div>
                      </div>
                    </div>
                  ...
                  <div className="text-center text-[var(--muted)] py-8">Хичээлээ сонгоно уу.</div>
                )}
              </div>
            )}
          </>
