...
function ParentPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const profileKey = `parent:profile:${studentId}`;
      const cached = getCache<StudentInfo>(profileKey, 1);
      if (cached) {
        setStudent(cached);
        setLoadingStudent(false);
        return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const resultsKey = `parent:results:${studentId}`;
  ...
  const overallAvg = useMemo(() => {
    ...
    const avgs = Object.values(subjRes)
      .map((r) => r.average)
  ...
  if (!accessChecked || user === undefined) {
    return (
      <div className="min-h-dvh flex items-center justify-center text-[var(--muted)] px-4">
        Эрх шалгаж байна…
      </div>
    );
  }
  ...
  if (err || !student) {
    return (
      <div className="min-h-dvh flex items-center justify-center px-4">
        <div
          className="p-5 sm:p-6 rounded-2xl max-w-lg w-full"
          style={{ background: "var(--card)", border: "1px solid var(--stroke)", color: "var(--text)" }}
        >
          <div className="font-bold mb-2">Алдаа</div>
          <div className="text-[var(--muted)]">
            Хүүхдийн мэдээлэл олдсонгүй. Таны имэйл сурагчийн бүртгэлтэй холбогдоогүй байна.
          </div>
  ...
  return (
    <div className="min-h-dvh bg-[var(--bg)] text-[var(--text)]">
      {/* Sticky top bar */}
      <div
        className="sticky top-0 z-30 px-4 py-3 border-b backdrop-blur supports-[backdrop-filter]:bg-[color:var(--card)/0.7] bg-[var(--card)]"
        style={{ borderColor: "var(--stroke)" }}
      >
        <div className="max-w-[1100px] mx-auto flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-3">
            <div className="text-xs font-medium text-[var(--muted)] bg-[var(--bg)] px-2 py-1 rounded-lg border border-[var(--stroke)]">
              Эцэг эх
            </div>
            <div className="text-base font-bold sm:text-lg">{student.name}</div>
            {student.externalId && (
              <div className="text-xs text-[var(--muted)]">Код: {student.externalId}</div>
            )}
          </div>
      ...
      <div className="max-w-[1100px] mx-auto px-4 py-4 sm:py-6">
          ...
          <div className="grid grid-cols-2 gap-3 w-full sm:w-auto">
            ...
            {subjectDetail && (
              <div
                ...
                style={{ background: "var(--bg)", border: "1px solid var(--stroke)" }}
              >
                <div className="text-[10px] sm:text-xs font-semibold text-[var(--muted)]">
                  Энэ хичээлийн дундаж
                </div>
                <div className="text-xl sm:text-2xl font-extrabold text-[var(--primary-bg)]">
        ...
        {subject && subjectDetail && (
          ...
          >
            ...
            <div className="overflow-x-auto -mx-2 sm:mx-0 px-2">
              <ReactApexChart
                key={`parent-${themeMode}-${studentId}-${subject}`}
                options={{
        ...
        {/* Radar chart */}
        <div
          className="rounded-2xl p-3 sm:p-4 mt-4 sm:mt-6 hidden sm:block"
          style={{ background: "var(--card)", border: "1px solid var(--stroke)" }}
        >
          <div className="text-sm sm:text-base font-bold mb-2 sm:mb-3">
            Хичээлүүдийн дундаж ({student.name})
          </div>
          <div className="overflow-x-auto">
            <ReactApexChart
              key={`parent-radar-${themeMode}-${studentId}`}
              options={{
                chart: { type: "radar", background: "transparent", toolbar: { show: false } },
                theme: { mode: themeMode },
                labels: subjects.filter((s) => (results[studentId]?.[s]?.average ?? 0) > 0),
                yaxis: { min: 0, max: 100, tickAmount: 5 },
              ...
              series={[
                {
                  name: "Дундаж оноо",
                  data: subjects
                    .filter((s) => (results[studentId]?.[s]?.average ?? 0) > 0)
...
export default withRole(ParentPage, ["parent"]);
